    ...
    <div class="flex flex-col-reverse md:flex-row relative">
      <div class="w-full md:w-2/3">
        <div class="flex flex-col gap-4 md:px-20 fade-zoom-up">
          <router-link v-for="article in articles" :key="article.url" :to="`/read/${article.slug}/${encodeURIComponent(article.url)}`" class="flex w-full bg-[#1e1e1f] border-[#383838] rounded-xl text-left text-white p-5 md:py-7 md:px-8 cursor-pointer hover:bg-[#282828] items-center">
            <div class="w-full pr-4">
              <h1 class="text-sm md:text-md text-sky-200 font-bold mb-2 paraf">{{ article.title }}</h1>
              <div class="text-sm hidden md:block paraf">{{ article.description }}</div>
              <div class="text-xs mb-1 text-slate-400 flex items-center italic">
                <div class="h-[1px] w-20 bg-sky-200 md:w-5 aos-init aos-animate mr-2"></div> {{ formatDate(article.publishedAt) }}
              </div>
            </div>
            <div>
              <div class="w-20 h-20 md:w-28 flex items-center md:h-28">
                <img :src="article.urlToImage || 'https://placekitten.com/200/200'" class="rounded-lg md:rounded-xl" alt="">
              </div>
            </div>
          </router-link>
        </div>
      ...
      <div class="w-full md:w-1/3 h-fit p-8 md:sticky md:top-24">
        ...
        <div class="flex flex-col text-left">
          <div class="bg-clip-text bg-gradient-to-r from-slate-100 to-sky-300 text-transparent">Let's share experiences, stories, and knowledge together.</div>
